...
enum MaterialNames {Skin, RedAcrylic, GreenJersey};
...
GLuint texture_vPos;
GLuint texture_vTex;
GLfloat incriment = 0.5f;
GLfloat incrimentbb = 0.1f;
...
GLfloat raiseTheCourt = 12.0f;
GLfloat daz = 2.0f;
GLint dir = 1;
GLfloat theta = 0.0f;
GLfloat elevation = 54.7f;
GLfloat basketballHeight = 0.0f;
GLfloat del = 2.0f;
GLfloat radius = 5.0f;
...
int main(int argc, char**argv)
{
    ...
    while ( !glfwWindowShouldClose( window ) ) {
        ...
        theta =theta+dT*rpm+incriment;
                right_upper_arm.set_update_transform(translate(TORSO_WIDTH/1.45f,2.5f+raiseTheCourt,0.0f)*rotate(-theta,1.0f,0.0f,0.0f)*translate(-(TORSO_WIDTH/1.45f),-(2.5f+raiseTheCourt),0.0f));
               left_upper_arm.set_update_transform(translate(TORSO_WIDTH/1.45f,2.5f+raiseTheCourt,0.0f)*rotate(theta,1.0f,0.0f,0.0f)*translate(-(TORSO_WIDTH/1.45f),-(2.5f+raiseTheCourt),0.0f));
        right_upper_leg.set_update_transform(translate(TORSO_WIDTH/3.5f,-6.0f+raiseTheCourt,0.0f)*rotate(theta,1.0f,0.0f,0.0f)*translate(-(TORSO_WIDTH/3.5f),-(-6.0f+raiseTheCourt),0.0f));

        left_upper_leg.set_update_transform(translate(TORSO_WIDTH/3.5f,-6.0f+raiseTheCourt,0.0f)*rotate(-theta,1.0f,0.0f,0.0f)*translate(-(TORSO_WIDTH/3.5f),-(-6.0f+raiseTheCourt),0.0f));
        basketball.set_update_transform(translate(-3.5f,-(2.5f-basketballHeight),-3.5f));
        basketball.set_update_transform( translate(3.5f,2.5f+basketballHeight,3.5f));
        basketballHeight = basketballHeight + (dT*rpm+incrimentbb/2.0f);
        if(basketballHeight>=7.0f)
        {
            incrimentbb = -incrimentbb;
        }
        if(basketballHeight<=0.0f)
        {
            incrimentbb = -incrimentbb;
        }
...
/*
        if(basketballHeight<=7.0f)
        {
           // basketball.set_update_transform(translate(vec3(0.0f,basketballHeight,0.0f)));
            while(basketballHeight<7.0f)
            {
                basketballHeight = basketballHeight+.02f*elTime/7;
                basketball.set_update_transform(translate(vec3(0.0f,basketballHeight,0.0f)));

            }

        }
        else if(basketballHeight>=7.0f)
        {
            //basketball.set_update_transform(translate(vec3(0.0f,basketballHeight,0.0f)));
            while(basketballHeight>.40f)
            {
                basketballHeight = basketballHeight-.02*elTime/7;
                basketball.set_update_transform(translate(vec3(0.0f,basketballHeight,0.0f)));
...
void build_texture_cube(GLuint obj) {
    ...
    uvCoords = {
            ...
            {(180.0f/512.0f),1.0f},
            {(180.0f/512.0f),0.0f},
            {(360.0f/512.0f),0.0f}, //back

            {1.0f, (29.0f / 256.0f)},
            {1.0f, 1.0f},
            {(436.0f / 512.0f), 1.0f},
            {(436.0f / 512.0f), 1.0f},
            {(436.0f / 512.0f), (29.0f / 256.0f)},
            {1.0f, (29.0f / 256.0f)}, //left

            {(436.0f/512.0f),(29.0f/256.0f)},
            {(436.0f/512.0f),1.0f},
            {(360.0f/512.0f),1.0f},
            {(360.0f/512.0f),1.0f},
            {(360.0f/512.0f), (29.0f / 256.0f)},
            {(436.0f/512.0f), (29.0f / 256.0f)}, //right
         ...
         /*   vec2((180.0f/512.0f),0.0f),
           vec2((180.0f/512.0f),1.0f),
           vec2(0.0f,1.0f),
           vec2(0.0f,1.0f),
           vec2(0.0f,0.0f),
           vec2((180.0f/512.0f),0.0f),//front

           vec2((360.0f/512.0f),0.0f),
           vec2((360.0f/512.0f),1.0f),
           vec2((180.0f/512.0f),1.0f),
           vec2((180.0f/512.0f),1.0f),
           vec2((180.0f/512.0f),0.0f),
           vec2((360.0f/512.0f),0.0f), //back

           vec2(1.0f,(29.0f/256.0f)),
           vec2(1.0f,1.0f),
           vec2((436.0f/512.0f),1.0f),
           vec2((436.0f/512.0f),1.0f),
           vec2((436.0f/512.0f),(29.0f/256.0f)),
           vec2(1.0f,(29.0f/256.0f)), //left

           vec2(1.0f,(29.0f/256.0f)),
           vec2(1.0f,1.0f),
           vec2((360.0f/512.0f),1.0f),
           vec2((360.0f/512.0f),1.0f),
           vec2((360.0f/512.0f),(29.0f/256.0f)),
           vec2(1.0f,(29.0f/256.0f)), //right
           ...
           vec2(0.0f,0.0f),*/
           /*
           vec2(1.0f,227.0f/256.0f),
           vec2(1.0f,1.0f),
           vec2(360.0f/512.0f,1.0f),
           vec2(360.0f/512.0f,1.0f),
           vec2(360.0f/512.0f,227.0f/256.0f),
           vec2(1.0f,227.0f/256.0f), //top
           ...
           vec2(360.0f/512.0f,1.0f),
           vec2(360.0f/512.0f,(227.0f/256.0f),
           vec2(1.0f,(227.0f/256.0f)), //bottom*/
    };
...
void build_materials( ) {
    ...
    MaterialProperties GreenJersey = {
            vec4(0.0f, 0.40f, 0.00f, 1.0f), //ambient
            vec4(0.0f, 0.77f, 0.0f, 1.0f), //diffuse
...
void build_scene_graph( ) {
    ...
    build_texture_node(torso, TexCube, Shirt, translate(0.0f,raiseTheCourt,0.0f)*scale(TORSO_WIDTH, TORSO_HEIGHT, TORSO_DEPTH));
   // torso.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    torso.attach_nodes(&head, &Court);


    build_texture_node(head, Sphere, Face,translate(0.0f,TORSO_HEIGHT/1.45f + raiseTheCourt,0.0f)*scale(HEAD_DEPTH, HEAD_HEIGHT, HEAD_WIDTH)*rotate(180.0f,1.0f,0.0f,0.0f));
  //  head.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    head.attach_nodes(NULL,&right_upper_arm);


    build_lighting_node(right_upper_arm, Cube, GreenJersey, false, translate(TORSO_WIDTH/1.45f,2.5f+raiseTheCourt,0.0f)*scale(UPPER_ARM_DEPTH, UPPER_ARM_HEIGHT, UPPER_ARM_WIDTH));
   // right_upper_arm.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));

    right_upper_arm.attach_nodes(&right_lower_arm, &left_upper_arm);


    build_lighting_node(left_upper_arm, Cube, GreenJersey, false, translate(-TORSO_WIDTH/1.45f,2.5f+raiseTheCourt,0.0f)*scale(UPPER_ARM_DEPTH, UPPER_ARM_HEIGHT, UPPER_ARM_WIDTH));
   // left_upper_arm.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    left_upper_arm.set_update_transform(rotate(-theta,1.0f,0.0f,0.0f));
    left_upper_arm.attach_nodes(&left_lower_arm, &left_upper_leg);


    build_lighting_node(right_lower_arm, Cube, Skin, false, translate(TORSO_WIDTH/1.45f,2.5f-UPPER_ARM_HEIGHT+raiseTheCourt,0.0f)*rotate(-15.0f,1.0f,0.0f,0.0f)*scale(LOWER_ARM_DEPTH, LOWER_ARM_HEIGHT, LOWER_ARM_WIDTH));

    right_lower_arm.attach_nodes(NULL,NULL);


    build_texture_node(Court,Cube, Wood, scale(30.0f,0.0f,30.0f));
    Court.attach_nodes(NULL, &basketball);

   // build_texture_node(basketball, Sphere, Basketball, translate(3.5f,2.5f+basketballHeight,3.5f)*scale(BALL_RADIUS,BALL_RADIUS,BALL_RADIUS));

    build_texture_node(basketball, Sphere, Basketball, scale(BALL_RADIUS,BALL_RADIUS,BALL_RADIUS));
    // basketball.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
   // basketball.set_update_transform( translate(3.5f,2.5f+basketballHeight,3.5f));
    basketball.attach_nodes(NULL, &cubebox);

    build_lighting_node(cubebox, Cube, RedAcrylic, true, translate(3.5f,2.5f,3.5f)*scale(BOX_WIDTH, BOX_HEIGHT, BOX_DEPTH));
   // cubebox.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    cubebox.attach_nodes(NULL, NULL);

    build_lighting_node(left_lower_arm, Cube, Skin, false, translate(-TORSO_WIDTH/1.45f,2.5f-UPPER_ARM_HEIGHT+raiseTheCourt,0.0f)*rotate(-15.0f,1.0f,0.0f,0.0f)*scale(LOWER_ARM_DEPTH, LOWER_ARM_HEIGHT, LOWER_ARM_WIDTH));
  //  left_lower_arm.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    left_lower_arm.attach_nodes(NULL,NULL);

    build_lighting_node(left_upper_leg, Cube, GreenJersey, false, translate(-TORSO_WIDTH/3.5f,-6.0f+raiseTheCourt,0.0f)* scale(UPPER_LEG_DEPTH,UPPER_LEG_HEIGHT,UPPER_LEG_WIDTH));
   // left_upper_leg.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    left_upper_leg.attach_nodes(&left_lower_leg, &right_upper_leg);


    build_lighting_node(right_upper_leg, Cube, GreenJersey, false, translate(TORSO_WIDTH/3.5f,-6.0f+raiseTheCourt,0.0f)* scale(UPPER_LEG_DEPTH,UPPER_LEG_HEIGHT,UPPER_LEG_WIDTH));
  //  right_upper_leg.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    right_upper_leg.attach_nodes(&right_lower_leg, NULL);

    build_lighting_node(right_lower_leg, Cube, Skin, false, translate(TORSO_WIDTH/3.5f,-6.0f-UPPER_LEG_HEIGHT+raiseTheCourt,0.0f)* rotate(15.0f,1.0f,0.0f,0.0f)*scale(LOWER_LEG_DEPTH,LOWER_LEG_HEIGHT,LOWER_LEG_WIDTH));
   // right_lower_leg.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
    right_lower_leg.attach_nodes(NULL,NULL);

    build_lighting_node(left_lower_leg, Cube, Skin, false, translate(-TORSO_WIDTH/3.55f,-6.0f-UPPER_LEG_HEIGHT+raiseTheCourt,0.0f)* rotate(15.0f,1.0f,0.0f,0.0f)*scale(LOWER_LEG_DEPTH,LOWER_LEG_HEIGHT,LOWER_LEG_WIDTH));
  //  left_lower_leg.set_update_transform(translate(0.0f,raiseTheCourt,0.0f));
